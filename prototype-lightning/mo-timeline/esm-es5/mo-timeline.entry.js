...
import { c as createCommonjsModule, h as hooks, a as commonjsGlobal, g as getCjsExportFromNamespace, T as TimeStepUtils } from './timestep-utils-e02ff818.js';
...
var baseTimelineCss = ".timelineContainer_normalView{background-color:#384F56;display:grid;display:-ms-grid;-ms-flex-pack:stretch;justify-content:stretch;color:#b9dc0c;font-size:16px;height:8rem;grid-template-areas:\"controlBar\" \"timeline\";grid-template-columns:1fr;grid-template-rows:0.5fr 1fr;-ms-grid-columns:1fr;-ms-grid-rows:0.5fr 1fr}.timelineContainer_normalView .timelineContainer__controlBar{grid-area:controlBar;display:-ms-grid;display:grid;text-align:center;grid-template-areas:\"play date slot\";grid-template-columns:1fr 4fr 1fr;-ms-grid-columns:1fr 4fr 1fr}.timelineContainer_normalView .timelineContainer__controlBar__playButtons{grid-area:play;text-align:left;width:-webkit-min-content;width:-moz-min-content;width:min-content;white-space:nowrap;-ms-grid-row:1;-ms-grid-column:1}.timelineContainer_normalView .timelineContainer__controlBar__dateLabel{grid-area:date;display:-ms-flexbox;display:flex;-ms-flex-pack:center;justify-content:center;-ms-flex-align:center;align-items:center;white-space:nowrap;-ms-grid-row:1;-ms-grid-column:2}.timelineContainer_normalView .timelineContainer__controlBar__slot{grid-area:slot;text-align:right;-ms-grid-row:1;-ms-grid-column:3}.timelineContainer_normalView .timelineContainer__timeline{grid-area:timeline;-ms-grid-row:2;padding:0 3rem}@media only screen and (max-width: 720px){.timelineContainer__timeline{padding:0 !important}.timelineContainer_normalView{height:5.5rem}}";
var BaseTimeline = /** @class */ (function () {
    function BaseTimeline(hostRef) {
        registerInstance(this, hostRef);
        this.newTimeStepSelected = createEvent(this, "newTimeStepSelected", 7);
        this.timeStepArray = [];
    ...
    BaseTimeline.prototype.constrainTimelineHandler = function () {
        this.timeLineConstraint = this.timeStepUtils.resetTimelineConstraint(this.timesteps.length);
    };
    BaseTimeline.prototype.onDefaultTimestepStateChange = function (newVal) {
        this.defaultTimestepState = newVal;
        this.disableTimesteps = (newVal === 'disabled') ? true : false;
    };
    BaseTimeline.prototype.onInitialTimeStepIndexChange = function (newVal) {
        this.timeStepIndex = newVal;
    };
    BaseTimeline.prototype.onTimeStepsChange = function (newVal) {
        this.timeStepArray = this.timeStepUtils.createSortedTimeStepArrayFromJSON(newVal);
        this.timeLineConstraint = this.timeStepUtils.resetTimelineConstraint(this.timesteps.length);
    };
    BaseTimeline.prototype.onLoadedTimeStepsChange = function (newVal) {
        if (newVal !== null) {
            ...
            var sortedLoadedTimeSteps = this.timeStepUtils.createSortedTimeStepArrayFromJSON(newVal);
    ...
    BaseTimeline.prototype.watchMouseState = function (newVal) {
        this.mouseState = newVal;
    };
    // Listen for changes to the constraint object (describes the left and right constraint values)
    BaseTimeline.prototype.constraintChangedHandler = function (event) {
        if (event.detail !== null) {
            this.timeLineConstraint = event.detail;
        }
    };
    BaseTimeline.prototype.dateClickedHandler = function (event) {
    ...
    BaseTimeline.prototype.forwardClickedHandler = function () {
        this.forward();
    };
    BaseTimeline.prototype.backClickedHandler = function () {
        this.backward();
    };
    BaseTimeline.prototype.playPauseClickedHandler = function () {
        this.play(!this.playing);
    };
    BaseTimeline.prototype.play = function (play) {
    ...
    BaseTimeline.prototype.mouseLeaveHandler = function (event) {
        var newMouseState = clone_1(this.mouseState);
        newMouseState.over = false;
        newMouseState.xPos = event.clientX;
        newMouseState.target = event.target;
        this.mouseState = newMouseState;
    };
    BaseTimeline.prototype.mouseOverHandler = function (event) {
    ...
    BaseTimeline.prototype.mouseDownHandler = function (event) {
        if (event.button === 0) {
            var newMouseState = clone_1(this.mouseState);
            newMouseState.down = true;
            newMouseState.up = false;
    ...
    BaseTimeline.prototype.mouseUpHandler = function (event) {
        if (event.button === 0) {
            var newMouseState = clone_1(this.mouseState);
    ...
    BaseTimeline.prototype.mouseMoveHandler = function (event) {
    ...
    BaseTimeline.prototype.componentWillLoad = function () {
    ...
    BaseTimeline.prototype.componentDidLoad = function () {
        var _this = this;
        // tslint:disable-next-line: no-unused-expression
        new ResizeSensor(this.element.firstElementChild, function () {
            _this.currentContainerWidth = _this.element.firstElementChild.getBoundingClientRect().width;
            if (_this.isMobileScreen()) {
                clearInterval(_this.timerId);
            }
        });
    };
    BaseTimeline.prototype.render = function () {
        return (h("div", { class: 'timelineContainer_normalView' }, h("div", { class: 'timelineContainer__controlBar' }, h("div", { class: 'timelineContainer__controlBar__playButtons' }, !this.isMobileScreen() &&
            h("play-buttons", { playing: this.playing, "disable-play-button": this.disablePlayButton })), h("div", { class: 'timelineContainer__controlBar__dateLabel' }, this.dateLabel((this.isMobileScreen()) ? 'ddd HH:mm zz' : this.dateLabelFormat)), h("div", { class: 'timelineContainer__controlBar__slot' }, h("slot", null))), this.timeStepArray.length > 0 &&
    ...
    BaseTimeline.prototype.setInitialSelectedTimeStep = function () {
        var newIndex = this.timeStepUtils.findNearestActiveTimeStep(this.timeStepIndex, this.timesteps.length, this.loadedTimeStepMap, this.timeLineConstraint);
        if (this.timeStepIndex !== newIndex) { // only emit the event if it is different
            this.timeStepIndex = newIndex;
            this.emitCurrentlySelectedTimeStep();
        }
    };
    BaseTimeline.prototype.isMobileScreen = function () {
        return (this.currentContainerWidth < this.mobileTimelineBreakPoint);
    };
    BaseTimeline.prototype.dateLabel = function (momentFormat) {
        var timezones = momentTimezone$1.tz.names();
        ...
        if (this.timeStepArray.length > 0) {
            return momentTimezone$1(this.timeStepArray[this.timeStepIndex]).tz(this.timezone).format(momentFormat);
        }
        else {
            return 'Timeline not available';
        }
    };
    BaseTimeline.prototype.renderMobileTimeline = function () {
    ...
    BaseTimeline.prototype.renderTimeline = function () {
    ...
    BaseTimeline.prototype.startTimer = function () {
        var _this = this;
        this.timerId = setInterval(function () {
            _this.forward();
        }, 1000);
    };
    BaseTimeline.prototype.forward = function () {
        var timeStepMap = this.disableTimesteps ? this.loadedTimeStepMap : null;
    ...
    BaseTimeline.prototype.backward = function () {
        var timeStepMap = this.disableTimesteps ? this.loadedTimeStepMap : null;
    ...
    BaseTimeline.prototype.emitCurrentlySelectedTimeStep = function () {
    ...
    Object.defineProperty(BaseTimeline.prototype, "element", {
        get: function () { return getElement(this); },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(BaseTimeline, "watchers", {
        get: function () {
            return {
                "constrainTimeline": ["constrainTimelineHandler"],
                "defaultTimestepState": ["onDefaultTimestepStateChange"],
                "timeStepIndex": ["onInitialTimeStepIndexChange"],
                "timesteps": ["onTimeStepsChange"],
                "loadedTimesteps": ["onLoadedTimeStepsChange"],
...
BaseTimeline.style = baseTimelineCss;
export { BaseTimeline as mo_timeline };
