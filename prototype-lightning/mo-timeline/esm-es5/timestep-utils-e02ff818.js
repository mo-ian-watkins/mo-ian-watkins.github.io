...
var TimeStepUtils = /** @class */ (function () {
    function TimeStepUtils() {
    }
    // Take a string representation of timesteps and converts to an Array.  Sort the array by time and remove any
    // duplicates
    TimeStepUtils.prototype.createSortedTimeStepArrayFromJSON = function (json) {
        var parsedTimeSteps = (typeof json !== 'object') ? JSON.parse(json) : json;
        var sortedTimeSteps = new Array();
        parsedTimeSteps.forEach(function (timeStep) {
            sortedTimeSteps.push(timeStep);
        });
        // don't assume the timeSteps are in date order, sort them
        sortedTimeSteps.sort();
        // remove any duplicates
        sortedTimeSteps = sortedTimeSteps.filter(function (item, index) {
    ...
    TimeStepUtils.prototype.createMatchingTimeStepMap = function (timeSteps, loadedTimeSteps) {
        var loadedTimeStepMap = new Map();
        var matchedTimestepIndex = 0;
        loadedTimeSteps.forEach(function (loadedTimeStep) {
    ...
    TimeStepUtils.prototype.findNearestNextActiveTimeStep = function (selectedIndex, numberOfTimeSteps, loadedTimeSteps, timeLineConstraint) {
    ...
    TimeStepUtils.prototype.findNearestPreviousActiveTimeStep = function (selectedIndex, numberOfTimeSteps, loadedTimeSteps, timeLineConstraint) {
    ...
    // being loaded then only the constraints are used to define if a timestep is active.
    TimeStepUtils.prototype.findNearestActiveTimeStep = function (selectedIndex, numberOfTimeSteps, loadedTimeSteps, timeLineConstraint) {
    ...
    TimeStepUtils.prototype.searchForNearestActiveTimeStep = function (selectedIndex, numberOfTimeSteps, loadedTimeSteps, timeLineConstraint, onlySearchForward, onlySearchBackwards) {
        var nearestTimeStepIndex = selectedIndex;
        if (loadedTimeSteps === null || loadedTimeSteps.size > 0) {
            ...
            if (this.isValidTimeStep((selectedIndex), loadedTimeSteps, timeLineConstraint)) {
                ...
                var foundNearbyTimestep = false;
                var outsideLeftBounds = false;
                var outsideRightBounds = false;
                var count = 0;
                // keep track of the total number of steps that we have checked.
                var totalStepsChecked = 0;
    ...
    TimeStepUtils.prototype.isValidTimeStep = function (index, loadedTimeSteps, timeLineConstraint) {
    ...
    TimeStepUtils.prototype.resetTimelineConstraint = function (numberOfTimeSteps) {
        return { left: 0, right: numberOfTimeSteps === 0 ? Number.MAX_SAFE_INTEGER : numberOfTimeSteps - 1 };
    };
    return TimeStepUtils;
}());
export { TimeStepUtils as T, commonjsGlobal as a, createCommonjsModule as c, getCjsExportFromNamespace as g, hooks as h };
