                ...
                if (this.enabled && this.pointerActive) {
                    ...
                    if (event.id === this.pointerId) {
                        ...
                        if (!this.pointerMoved) {
                            var movingVertically = Math.abs(Math.abs(this.pointerCurrentX) - Math.abs(this.pointerLastX)) <
                                Math.abs(Math.abs(this.pointerCurrentY) - Math.abs(this.pointerLastY));
                            if ((this.o.vertical && movingVertically) ||
...
var MobileTimeline = /** @class */ (function () {
    ...
    MobileTimeline.prototype.componentDidLoad = function () {
        this.generateSlider(this.timeSteps, this.loadedTimeStepMap);
    };
    MobileTimeline.prototype.onTimeStepIndexChange = function (newTimeStepIndex) {
        if (this.timeline) {
    ...
    MobileTimeline.prototype.onTimeStepsChange = function (newTimeSteps) {
        var slider = this.el.firstElementChild.querySelector('.ms-container');
        this.el.firstElementChild.removeChild(slider);
        this.timeline = null;
        this.generateSlider(newTimeSteps, this.loadedTimeStepMap);
    };
    MobileTimeline.prototype.onLoadedTimeStepsChange = function (newTimeSteps) {
    ...
    MobileTimeline.prototype.newIndex = function (event) {
        if (this.timeline) {
    ...
    MobileTimeline.prototype.generateSlider = function (timesteps, loadedTimesteps) {
        var numberOfTimeSteps = timesteps.length;
        var newSelection = this.newIndexSelected;
        var getNewIndex = this.getNewIndex;
        ...
        this.timeline = new momentumSlider({
            ...
            change: function (newIndex, _oldIndex) {
                var index = getNewIndex(newIndex, numberOfTimeSteps);
                var slides = this.el.firstElementChild.querySelectorAll('.timestep');
    ...
    MobileTimeline.prototype.getNewIndex = function (newIndex, numberOfTimeSteps) {
        var index;
    ...
    MobileTimeline.prototype.render = function () {
        return (h("div", { class: 'container' }));
    };
    Object.defineProperty(MobileTimeline.prototype, "el", {
        get: function () { return getElement(this); },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(MobileTimeline, "watchers", {
        get: function () {
            return {
...
var playButtonsCss = ".buttonsContainer .buttonsContainer__button{background-color:transparent;height:46.5px;width:46.5px;border:none;text-align:-webkit-center;cursor:pointer}.buttonsContainer .buttonsContainer__button svg{display:block;height:60%}";
var PlayButtons = /** @class */ (function () {
    function PlayButtons(hostRef) {
        registerInstance(this, hostRef);
    ...
    PlayButtons.prototype.onpPlayingChange = function (newVal) {
        this.playing = newVal;
    };
    PlayButtons.prototype.render = function () {
        var _this = this;
        return (h("div", { class: 'buttonsContainer', role: 'controlbuttons' }, h("button", { class: 'buttonsContainer__button back', title: 'Back', "aria-label": 'Back', onClick: function () {
            ...
            h("button", { class: 'buttonsContainer__button play', title: this.playing ? 'Pause' : 'Play', "aria-label": 'PlayPause', onClick: function () {
                    _this.handlePlayPauseClick();
                } }, (this.playing) ?
                ...
                :
                    h("svg", { xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 512 512' }, h("path", { d: 'M96 52v408l320-204L96 52z', fill: '#B9DC0C' }))), h("button", { class: 'buttonsContainer__button forward', title: 'Forward', "aria-label": 'Forward', onClick: function () {
                _this.handleForwardClick();
    ...
    PlayButtons.prototype.handleBackClick = function () {
        this.backClicked.emit();
    };
    PlayButtons.prototype.handlePlayPauseClick = function () {
        this.playPauseClicked.emit();
    };
    PlayButtons.prototype.handleForwardClick = function () {
        this.forwardClicked.emit();
    };
    Object.defineProperty(PlayButtons, "watchers", {
        get: function () {
            return {
...
var timeLineCss = ".timesteps{white-space:initial;position:relative}";
var Timeline = /** @class */ (function () {
    ...
    Timeline.prototype.constrainTimelineHandler = function () {
        this.timeLineConstraint = this.timeStepUtils.resetTimelineConstraint(this.timeSteps.length);
    };
    Timeline.prototype.onTimeStepsChange = function (newTimesteps) {
    ...
    Timeline.prototype.watchMouseState = function (newVal) {
        this.mouseState = newVal;
    };
    Timeline.prototype.constraintChangedHandler = function (event) {
        if (event.detail !== null) {
            this.timeLineConstraint = event.detail;
        }
    };
    Timeline.prototype.componentWillLoad = function () {
    ...
    Timeline.prototype.render = function () {
        ...
        if (this.timeSteps) {
            return (h("div", { class: 'timesteps' }, h("time-slider", { timeSteps: this.timeSteps, timeStepProperties: this.timeStepProperties, loadedTimeStepMap: this.loadedTimeStepMap, timeStepIndex: this.timeStepIndex, disableTimesteps: this.disableTimesteps, mouseState: this.mouseState, constrainTimeline: this.constrainTimeline }), (this.warnings && this.warnings.length > 0) &&
                h("timeline-warnings", { firstTimeStep: this.timeSteps[0], lastTimeStep: this.timeSteps[this.timeSteps.length - 1], warnings: this.warnings }), this.timeSteps.map(function (timeStep, index) {
                return (h("time-step", { timeData: _this.generateTimeStepProp(timeStep, index, _this.constrainTimeline), labelFormat: _this.labelFormat }));
            })));
        }
        else {
            return null;
    ...
    Timeline.prototype.generateTimeStepProp = function (time, timeStepIndex, constrainTimeline) {
        var timeStepProperty = this.timeStepProperties[timeStepIndex];
        ...
        if (timeStepProperty === null || typeof timeStepProperty === 'undefined') {
            var timeStep = hooks(time);
            var previousTimeStep = (timeStepIndex > 0) ? hooks(this.timeSteps[timeStepIndex - 1]) : undefined;
            var maxIndex = this.timeSteps.length - 1;
            var nextTimeStep = (timeStepIndex < maxIndex) ? hooks(this.timeSteps[timeStepIndex + 1]) : undefined;
            timeStepProperty = {
        ...
        var timeStepEnabled = timeStepProperty.enabled;
    ...
    Object.defineProperty(Timeline, "watchers", {
        get: function () {
            return {
